...
d = 0.000000000000000000000000000000000000123; // TODO: потеряются ли 23 в конце? почему? 1.23e-26
d = 1.; //1
...
if (ch < 65 || (ch > 'Z' && ch < 'a') || ch > 122){
    ...
    cout << "Ваш символ в другом регистре: " << ch << endl;
}
}
...
{
    //х должен быть вещественным, иначе не будет ситуации с 0 < x < 1
    //y должен быть вещественным, тк есть случай, в котором его приравнивают к вещественному x
    using namespace std;
    double x, y;
    cout << "Введите x: ";
    cin >> x;
...
/** проверить корректность значений */
    if (up < down){
        cout << "Нижняя граница больше верхней!";
    }
/** вычислить сумму */
    int summa = 0;
    for (down; down < up; down++){
        summa += down;
    }
...

{
    ...
    int iInput;
    do{
        cout << "Введите целое число >= 10 и чётное: ";
        cin >> iInput;
    } while (!(iInput >= 10 && iInput % 2 == 0));
}
...

{
    ...
    std::cout << "sum > 1.7 при x равном " << x;
}
